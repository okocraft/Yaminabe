...
If a target selector contains at least one player other than the command executor, the corresponding `.others` permission
is required and the entire operation is rejected when it is missing. A non-player executor always requires `.others`.
